...
class SaferesourceController extends Controller {
	...
	public function knowledge_category_page(){
		...
		$all_data=$web_knowledge_category_one;
		
		$m['active']=0;
		$m['knowledge_category_id']=$all_data['id'];
		$all_data['data_info']=M("web_safe_resource")->where($m)->select();
		
		$web_safe_resource = M("web_safe_resource")->where($m)->limit(3)->select();
		$this->assign('web_safe_resource',$web_safe_resource);
