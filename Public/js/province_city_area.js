...
var init = function(){
	...
	for(var i=0;i<province_enum.length;i++)	{
		htm+= ('<option value='+province_enum[i].id+'>'+province_enum[i].name+'</option>')
	}
	$('#select_province').html(htm)
	var pro_id = $('#select_province').val()
	var cities = province_enum[pro_id-1].city
	var areas =cities[0].district
	//alert(areas[0].name);
	
	var htm = ''
	for(var i=0;i<cities.length;i++)	{
		htm+= ('<option value='+cities[i].id+'>'+cities[i].name+'</option>')
	}
	$('#select_city').html(htm);
	
	var htm = ''
	for(var i=0;i<areas.length;i++)	{
		htm+= ('<option value='+areas[i].id+'>'+areas[i].name+'</option>')
	}
	...
	$('#select_city').change(function(){
		...
		for(var i=0;i<cities.length;i++){
			if(cities[i].id==city_id){
				var district = cities[i].district
				var htm = ''
				for(var i=0;i<district.length;i++)	{
					htm+= ('<option value='+district[i].id+'>'+district[i].name+'</option>')
				}
				$('#select_area').html(htm)
			}
		}
