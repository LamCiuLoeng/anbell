...
def init():
    ...
    rule37 = Rule( name = 'statistic_admin_view', title = '管理员统计管理', )
    rule38 = Rule( name = 'statistic_teacher_view', title = '老师统计管理', )
    ...
    gAdmin.rules = ",".join( map( str, [rule01.id, rule02.id, rule03.id, rule04.id, rule05.id, rule06.id, rule07.id, rule08.id, rule09.id,
                             ...
                             rule30.id, rule31.id, rule32.id, rule33.id, rule34.id, rule35.id, rule36.id, rule37.id
                             ] ) )
    gTeacher = Group( title = 'TEACHER' , display_name = '老师' )
    gTeacher.users = [teacher, ]
    gTeacher.rules = ",".join( map( str, [rule01.id, rule02.id, rule03.id, rule04.id, rule05.id, rule06.id, rule36.id, rule38.id
                               ] ) )
